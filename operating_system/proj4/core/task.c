...
int32u_t get_highest_priority(){
	int i;
	for(i=0; i<=LOWEST_PRIORITY; i++){
		if(_os_ready_queue[i] && _os_ready_queue[i]->next) {break;}
	}
	return i;
}
...
//	print_ready_queue();
	...
	int32u_t highest_priority = get_highest_priority();
...
//		PRINT("test: current_task: %x, highest_priority: %d\n", _os_ready_queue[_os_get_highest_priority()]->next->ptr_data, _os_get_highest_priority());
		_os_current_task = _os_ready_queue[highest_priority]->next->ptr_data;
		_os_current_task->state = 0;
		pop_ready_queue(_os_ready_queue[highest_priority]);
	...
	_os_next_task = _os_ready_queue[highest_priority]->next->ptr_data;
...
//	PRINT("schedule, current_task %x, next_task %x (priority %d)\n", _os_current_task, _os_next_task, highest_priority);
...
// implemented for resuming task after semaphore waiting queue
int32u_t eos_resume_task(eos_tcb_t *task) {
//	PRINT("  eos_resume_task\n");

	// change state into ready and perform scheduling
	task->state = 1;
	push_ready_queue(_os_ready_queue[task->priority], task);
	eos_schedule();
