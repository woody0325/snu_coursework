...
typedef struct eos_semaphore {
	int32u_t count; // number of resource available
	_os_node_t *wait_queue;
	int8u_t queue_type; // 0: FIFO, 1: priority-base
	int8u_t timeout; // made for timeout of eos_acquire_sempaohore function
} eos_semaphore_t;
...
typedef struct eos_mqueue {
	...
	void *rear; // last element in queue
	int8u_t queue_type; // 0: FIFO, 1: priority-base
	eos_semaphore_t putsem; // semaphore for resource management of sending msg
	eos_semaphore_t getsem; // semaphore for resourece management of receiving msg
} eos_mqueue_t;
