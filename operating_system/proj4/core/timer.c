	...
	while(1){
		...
		if(!queue->next){
			...
			if(ptr_alarm->timeout > node->timeout){
				...
				if(((eos_tcb_t*)ptr_alarm->arg)->priority > ((eos_tcb_t*)node->arg)->priority){
					break;
				}
			}
		}
...
void print_alarm_queue(eos_counter_t *counter){
	...
	while(1){
		if(!ptr_queue) break;
		eos_tcb_t *ptr_tcb = (eos_tcb_t*)((eos_alarm_t*)ptr_queue->ptr_data)->arg;
		printf("  element: %x, timeout: %d, priority: %d\n", ptr_tcb, ((eos_alarm_t*)ptr_queue->ptr_data)->timeout, ptr_tcb->priority);
...
//	PRINT("eos_set_alarm, tcb: %x, timeoute: %d\n", arg, timeout);
